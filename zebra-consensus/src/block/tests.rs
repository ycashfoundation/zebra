...
fn equihash_is_valid_for_historical_blocks() -> Result<(), Report> {
    ...
    for block in block_iter {
        ...
        // Resolve `(N, K)` from the block's own coinbase height so both
        // pre-fork Zcash-style (200, 9) blocks and post-UPGRADE_YCASH
        // (192, 7) blocks validate under their own parameters.
        let height = block
            .coinbase_height()
            .expect("block has a parseable coinbase height");
        check::equihash_solution_is_valid(&block.header, &Network::Mainnet, height)
